...
## Inputs
  ...
  ### `rule-priority-range`:
   **Optional** range of the NSG priority values to be used when multiple agents are deploying sharing the same NSG. Default `"100"`.
  ### `rule-inbound-port`:
   **Optional** Port for the inbound rule'. Default `"443"`.
  ### `rule-id-for-removal`:
   **Optional** rule id to remove. If this value is defined the action will default to deleteing. Default `""`.
  ### `rule-nsg-resource-group-name`:
   **Required** Resource Group of the NSG. Default `"N/A"`.
  ### `rule-nsg-name`:
   **Required** Name of the NSG. Default `"N/A"`.
